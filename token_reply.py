...
def get_hidden_transitions_to_enable(marking, places_with_missing, places_shortest_path_by_hidden):
    ...
    for p1 in marking_places:
        for p2 in places_with_missing_keys:
            ans = build_condition_for_hidden(p1,p2,places_shortest_path_by_hidden)
            if ans[0]!=None:
                hidden_transitions_to_enable.append(places_shortest_path_by_hidden[ans[0]][ans[1]])
    hidden_transitions_to_enable = sorted(hidden_transitions_to_enable, key=lambda x: len(x))

    return hidden_transitions_to_enable

def build_condition_for_hidden(p1,p2,places_shortest_path_by_hidden):
    for place in places_shortest_path_by_hidden:
        if place.name == p1.name:
            for dest_place in places_shortest_path_by_hidden[place]:
                if dest_place.name == p2.name:
                    return place,dest_place
    return None, None
