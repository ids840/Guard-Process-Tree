...
def return_names(group_of_tp):
    names = []
    for tp in group_of_tp:
        names.append(tp.name)
    return names
def return_tp(marking,name):
    for place in marking:
        if place!=None and place.name == name:
            return place
def place_by_name(net,place_name):
    for place in net.places:
        if place.name == place_name:
            return place
def is_enabled(t, pn, m):
    if t.name not in return_names(pn.transitions):
        return False
    else:
        for a in t.in_arcs:
            find_place = return_tp(m,a.source.name)
            if find_place == None or m[find_place] < a.weight:
                return False
    return True
...
def execute(t, pn, m):
    ...
    for a in t.in_arcs:
        find_place = return_tp(m, a.source.name)
        m_out[find_place] -= a.weight
        if m_out[find_place] == 0:
            del m_out[find_place]

    for a in t.out_arcs:
        find_place = place_by_name(pn,a.target.name)
        m_out[find_place] += a.weight
